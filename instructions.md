...
Только учитывайте, что между п.6 и 7 инструкции сменится сеть, которую раздает роутер (на прошивке GL-inet сеть 192.168.8.0/24, а на родной OpenWRT - 192.168.1.0/24). Поэтому после окончания прошивки и до Do a happy dance нужно в браузере поменять адрес, по которому обращаться на роутер с 192.168.8.1 на 192.168.1.1
